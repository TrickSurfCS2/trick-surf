...
import { API_URL, OTEL_EXPORTER_OTLP_ENDPOINT } from '../lib/env'
...
const SERVICE_NAME = 'trick-surf-client'
 ...
 *
 * При отсутствии сети FetchTransport возвращает retryable-ошибку,
 * поверх которой RetryingTransport делает до 5 попыток с бэкоффом.
 *
 * Обёртка перехватывает fetch до того, как транспорт его захватит, и для OTLP-эндпоинтов:
 *  - если браузер оффлайн (navigator.onLine === false) — мгновенно резолвит 2xx;
 *  - если сетевой запрос упал — резолвит 2xx без лишних ретраев.
 */
function installOfflineOtlpGate() {
  ...
  const isOtlp = (input: RequestInfo | URL): boolean => {
    ...
    return Boolean(url && otlpHostPattern.test(url))
  }
    ...
    const observer = new PerformanceObserver((list) => {
      for (const entry of list.getEntries()) {
        ...
        for (const st of serverTiming) {
          timingMetrics[st.name] = st.duration
        }

        if (Object.keys(timingMetrics).length === 0)
          continue

        for (const [metric, duration] of Object.entries(timingMetrics)) {
...
function setupWebVitals(vitals: WebVitalsInstruments) {
  ...
  if ('PerformanceObserver' in window) {
    try {
      ...
      // first-input не поддерживается браузером
    }
  }
}

/**
 * Инициализирует OpenTelemetry Web SDK (стек SigNoz):
...
export function initMonitoring() {
  ...
  setupWebVitals({
    ...
    fid: webVitalsMeter.createHistogram('fid', { description: 'First Input Delay', unit: 'ms' }),
  ...
  registerInstrumentations({
    instrumentations: [
      ...
      new FetchInstrumentation({
        propagateTraceHeaderCorsUrls: apiTracePattern ? [apiTracePattern] : undefined,
      }),
...
export function setTelemetryUser(userData: { id: string | number, username?: string }) {
  userAttributes = {
